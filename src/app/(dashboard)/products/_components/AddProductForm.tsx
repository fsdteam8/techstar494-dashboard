...
export default function AddProductForm({
    ...
    if (isEditing && editProduct) {
      ...
      setExpirationDate(
        editProduct.expirationDate
          ? new Date(editProduct.expirationDate)
          : undefined
      );
      ...
      setRestrictedStates(
        editProduct.restrictedStates?.length
          ? editProduct.restrictedStates
          : [""]
      );
  ...
  const handleSubmit = async () => {
    ...
    formDataToSend.append(
      "restrictedStates",
      JSON.stringify(restrictedStates.filter((state) => state.trim() !== ""))
    );
    if (expirationDate)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 flex flex-col gap-6 shadow-[0_0_10px_2px_#1018281A] rounded-[8px]">
          <Card className="border-none shadow-none">
            <CardHeader>
              <CardTitle className="text-xl text-[#1A1C21] font-semibold">
                General Information
              </CardTitle>
            </CardHeader>
            <CardContent className="grid gap-4">
              <div>
                <label
                  htmlFor="name"
                  className="block text-base font-medium text-[#272727] mb-1"
                >
                  Product Name
                </label>
                <Input
                ...
                {restrictedStates.map((state, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <Input
                      ...
                      value={state}
                      onChange={(e) =>
                        updateRestrictedState(index, e.target.value)
                      }
                      className="h-[40px] border border-[#707070] rounded-[8px]"
                    />
