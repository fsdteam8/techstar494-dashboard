...
const fetchProducts = async (page: number, searchQuery: string = ""): Promise<ApiResponse> => {
  const capitalizedSearchQuery = searchQuery.charAt(0).toUpperCase() + searchQuery.slice(1);

  const response = await fetch(
    `${process.env.NEXT_PUBLIC_BACKEND_URL}/products?search=${encodeURIComponent(capitalizedSearchQuery)}&page=${page}`
  )
  if (!response.ok) {
...
export default function ProductsTable({ onEditProduct, searchQuery = "" }: ProductsTableProps) {
          ...
          <div className="divide-y divide-gray-100">
            {data?.data?.map((product) => (
              ...
              >
                ...
                <div className="text-sm text-[#111827] font-medium md:block col-span-1 flex">
                  {product.prices.length > 0 ? (
                    product.prices.map((price, index) => (
                      <div key={index} className="flex gap-2">
                        <span className="text-[#111827] text-sm font-medium hidden md:inline">
                    ...
                    <span className="text-gray-700 font-medium hidden md:inline">No prices available</span>
                  )}
                </div>
